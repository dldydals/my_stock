...
// 타입 정의 (types.ts 파일이 있다면 거기서 가져와도 되지만, 일단 여기에 유지)
interface StockData {
  key: string;
  ticker: string;
  name: string;
            ...
            <div className="p-4 relative z-10" style={headerHeatmapStyle}>
              <div className="grid grid-cols-[1fr_auto_auto] gap-4 items-center">
                ...
                <div className="flex flex-col items-end justify-center pr-2 border-r border-white/10">
                    ...
                    <Popover
                      ...
                      title={
                        <Text strong className="text-xs">
                          <MessageOutlined className="mr-1" /> AI 분석
                        </Text>
                      }
                      trigger="hover"
                    >
                      <Tag
                        color={outlookColor}
                        className="rounded-md px-1.5 py-0 border-none cursor-help text-[9px] m-0 shadow-sm font-bold h-[18px] leading-[18px]"
                      >
                        {analysis.outlook}
                      </Tag>
                    </Popover>
                  ...
                  <div
                    className={`flex items-center gap-1 mt-1 px-2 py-0.5 rounded ${item.changeRate > 0 ? "bg-red-500/20" : item.changeRate < 0 ? "bg-blue-500/20" : "bg-slate-500/20"}`}
                  >
                    <Text
                      className="font-numeric"
                      style={{
                    ...
                    >
                      ...
                      {Math.abs(
                        (item.currentPrice * (item.changeRate / 100)) /
                          (1 + item.changeRate / 100),
                      )
                        .toFixed(0)
                        .toLocaleString()}
                    </Text>
                ...
                <div className="flex flex-col items-end justify-center gap-0.5">
                  <div className="flex items-center justify-end gap-2">
                    <Text style={{ fontSize: 10, fontWeight: 600, color: "#93c5fd" }}>
                      단기매수
                    </Text>
                    <Text
                      className="font-numeric"
                    ...
                    >
                      {(analysis?.buy_price || 0) > 0
                        ? (analysis?.buy_price || 0).toLocaleString()
                        : "-"}
                    </Text>
                  </div>
                  <div className="flex items-center justify-end gap-2">
                    ...
                    >
                      {(analysis?.target_price || 0) > 0
                        ? (analysis?.target_price || 0).toLocaleString()
                        : "-"}
                    </Text>
                  </div>
                  <div className="flex items-center justify-end gap-2">
                    ...
                    >
                      {(analysis?.stop_loss || 0) > 0
                        ? (analysis?.stop_loss || 0).toLocaleString()
                        : "-"}
                    </Text>
                  </div>
                </div>
            ...
            <div className="p-4">
              ...
              <div className="flex justify-between items-end mb-4 relative z-10">
                ...
                <div className="flex flex-col items-end">
                  <Text
                    type="secondary"
                    style={{ fontSize: 10, fontWeight: 700, color: "#e2e8f0" }}
                  >
                    수익 (손익률)
                  </Text>
                  <div className="flex flex-col items-end">
                    <Text
                      ...
                      style={{
                        fontSize: 16,
                        fontWeight: 800,
                        color: item.yield > 0 ? "#fca5a5" : "#93c5fd",
                      }}
                    >
              ...
              {strat && (
                <div className="mb-4">
                  <div className="bg-black/20 rounded-xl p-2 border border-white/5 flex flex-col gap-1">
                    
                    {/* 1. 상단: 전략 제목(좌) - 매매신호(우) */}
                    <div className="flex justify-between items-center">
                      <Space size={4}>
                        <ThunderboltFilled style={{ fontSize: 10, color: "#fbbf24" }} />
                        <Text style={{ fontSize: 10, color: "#e2e8f0", fontWeight: 700 }}>
                          Strategy
                        </Text>
                      </Space>
                      <div>
                        {strat.action ? (
                          <Tag
                            color={strat.action === "BUY" ? "red" : strat.action === "SELL" ? "blue" : "default"}
                            style={{ margin: 0, fontSize: 10, fontWeight: 800, padding: "0 6px" }}
                          >
                            {strat.action}
                          </Tag>
                        ) : (
                    ...
                    <div className="flex justify-between items-center">
                      <Tooltip
                        title={
                          <div className="text-xs space-y-1">
                            ...
                            <div className="flex items-center gap-2">
                              <span className="text-blue-300 font-bold">30↓</span>{" "}
                              <span>과매도 (매수 고려)</span>
                            </div>
                          </div>
                        }
                      ...
                      >
                        {strat.rsi !== undefined && strat.rsi !== null
                          ? strat.rsi.toFixed(1)
                          : "-"}
                      </Text>
                    </div>
